...
/**
 * Created by manjooralam on 3/5/2018.
 */

public class PaymentShowListAdapter extends BaseAdapter {
    Context context;
    private List<Map> mListMap;

    public PaymentShowListAdapter(Context context, List<Map> mListMap) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        PaymentShowListAdapter.ViewHolder holder = null;
        if (convertView == null) {
            convertView = LayoutInflater.from(parent.getContext()).inflate(R.layout.row_add_credit_note_item, parent, false);
            holder = new PaymentShowListAdapter.ViewHolder(convertView);
            convertView.setTag(holder);
        } else {
            holder = (PaymentShowListAdapter.ViewHolder) convertView.getTag();
        }
        if ( mListMap.get(position).get("goodsItem").equals("")){
