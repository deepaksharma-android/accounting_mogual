...
public class AEMScrybeDevice 
{
	...
	public ArrayList<String> getPairedPrinters() 
	{
		...
		for (BluetoothDevice device : pairedDevices) {
			...
			if ((device.getName().contains("rinter"))||(device.getName().contains("Teklogik"))||(device.getName().contains("MP57"))||(device.getName().contains("PP801"))||(device.getName().contains("L51"))||(device.getName().contains("MPT"))||(device.getName().contains("Rfid")))
